...
#include <stdlib.h>
#include "opera.h"
dt *KronPro(dt *A,dt *B,int a,int b,int c,int d){
	dt *res = new dt[a*b*c*d];
	dt *d_A;
	dt *d_B;
	dt *d_res;
	cudaMalloc((void**)&d_A,sizeof(dt)*a*b);
	cudaMalloc((void**)&d_B,sizeof(dt)*c*d);
	cudaMalloc((void**)&d_res,sizeof(dt)*a*b*c*d);
	cudaMemcpy(d_A,A,sizeof(dt)*a*b,cudaMemcpyHostToDevice);
	cudaMemcpy(d_B,B,sizeof(dt)*c*d,cudaMemcpyHostToDevice);
	dim3 threads(512,1,1);
	dim3 blocks((a*b*c*d+512-1)/512,1,1);
	kronpro<<<blocks,threads>>>(d_A,d_B,d_res,a,b,c,d);
	cudaMemcpy(res,d_res,sizeof(dt)*a*b*c*d,cudaMemcpyDeviceToHost);
	return res;
}
...
*/
	clock_t start1,end1,end2,end3;
	start1 = clock();
	Btensor2mat(A,A1,A2,A3,a,b,c);
	end1 = clock();
	cout<<"张量变矩阵"<<"  ";
	cout<<(double)(end1-start1)/CLOCKS_PER_SEC<<"s"<<endl;
...
//	msvd(A3,U3,c,a*b,r3);	//c*r3
	...
	end3 = clock();
	cout<<"batch cheng"<<"  ";
	cout<<(double)(end3-end2)/CLOCKS_PER_SEC<<"s"<<endl;
...
*/
    ...
    t3 = clock();
    cout<<"释放空间"<<"  ";
	cout<<(double)(t3-end3)/CLOCKS_PER_SEC<<"s"<<endl;
	}
...
// m*r  n*r  m*n*r
	...
	transpose<<<blocks1,threads1>>>(d_M,d_MT,m,r);
	dim3 threads2(512,1,1);
	dim3 blocks2((n*r+512-1)/512,1,1);
	transpose<<<blocks2,threads2>>>(d_N,d_NT,n,r);
	//now d_MT*M  d_NT*N
	dt *d_MTM;
	...
	elepro<<<blocks3,threads3>>>(d_MTM,d_NTN,r);
	...
	if(flag == 1){
		mode1tran<<<blocks4,threads4>>>(d_X,d_X_M,k,n,m);
	}else if(flag == 2){
		mode2tran<<<blocks4,threads4>>>(d_X,d_X_M,n,k,m);
	}else{
		mode3tran<<<blocks4,threads4>>>(d_X,d_X_M,n,m,k);
	}
	cudaFree(d_X);
